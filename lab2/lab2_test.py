...
def test_image(image_file, image):
    ...
    #Images need to be normalized for last model, since we moved this part out
    #to preprocessing
    if(np.max(test_array)>1):
        test_array = (test_array*1.0)/255
...
for image in image_files:
    pil_image=tf.keras.preprocessing.image.load_img(image, target_size=(IMG_SIZE, IMG_SIZE))
    #pil_image.show()
